...
//    auto object_to_world = make_scaling_matrix_xyz(vec3f(2.0f,2.0f,0));
    // make rotation
    object_to_world = make_rotation_matrix({0,1,0}, 50.0f);
    if (!is_rotation_matrix(object_to_world)){
        cout << "matrix is not a rotation" << endl;
        return 1;
    };
    // TODO: TEST 3D object ready, write a index ver0.1

    auto mat1 = make_shared<lambertian>(pixel_f(1.0, 0.5, 0.0));
    auto mat2 = make_shared<lambertian>(pixel_f(1.0f, 0.2f, 0.0f));

    auto pyr = pyramid_sqr_base(vec3f(-2,-3, 5), vec3f(4, 0, 0), vec3f(0, 0,-4), vec3f(0,1,0), mat1);
    for (int i=0; i < pyr->objects_list.size(); i++){
        auto triangle = pyr->objects_list[i];
        triangle->apply_w2o();
    }
    world.append(pyr);

    camera main_camera;
    main_camera.image_width = 500;
    main_camera.aspect_ratio = 1.f;
    main_camera.look_from = vec3f(0,4,8);
...
//    txt_out.close();
    ...
    main_camera.render_png("rotation_test.png", world);
